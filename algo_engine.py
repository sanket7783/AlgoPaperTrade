...
class AlgoTradingEngine:
    ...
    def __init__(self, config: AppConfig):
        ...
        self.enforce_market_hours = False  # Set True to restrict trading to 09:00 AM - 11:30 PM IST
    ...
    def update_config(self, new_config: AppConfig):
        ...
        self.oanda_client.api_token = new_config.oanda.api_token
        self.oanda_client.account_id = new_config.oanda.account_id
        self.oanda_client.environment = new_config.oanda.environment
    ...
    def process_tick(self) -> Dict[str, Any]:
        ...
        2. Fetches Forex XAU_USD candles from OANDA (1m or 5m as configured).
        3. Fetches live MCX price from Groww API if authenticated; otherwise converts XAU_USD to MCX.
        4. Evaluates current strategy signals.
        5. Checks Stop-loss/Take-profit on open positions.
        ...
        df_candles = self.oanda_client.fetch_candles(instrument=instrument, granularity=timeframe, count=100)
        ...
        if df_candles is not None and not df_candles.empty:
            ...
            self.latest_forex_price = self.oanda_client.fetch_latest_price(instrument)

        # 2. Get MCX Price (Live Groww API vs OANDA Converted)
        ...
        # 3. Check tick SL/TP on active positions
        ...
        if self.enforce_market_hours and not market_open:
            signal_res = {"signal": "NEUTRAL", "reason": "MCX Market Closed (Open 09:00-23:30 IST)", "confidence": 0.0}
            self.latest_signal = signal_res
